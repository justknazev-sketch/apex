    ...
    <nav className="site-nav">
      <div className="nav-container">
        ...
        <ul className="nav-menu">
          ...
          <li><a href="#reviews" onClick={(e) => handleNavClick(e, '#reviews')}><MessageSquare size={16} />{t('nav_reviews')}</a></li>
          <li><a href="#contact" onClick={(e) => handleNavClick(e, '#contact')}><Phone size={16} />{t('nav_contact')}</a></li>
          {isAdminLoggedIn && (
            <li>
        ...
        <div className="nav-actions">
          {/* Language Switcher */}
          <div 
            className={`lang-dropdown-wrapper ${langOpen ? 'open' : ''}`}
          ...
          >
            ...
            {langOpen && (
              <div className="lang-dropdown-menu">
                <button onClick={() => { setLanguage('uk'); setLangOpen(false); }} className={`lang-opt ${language === 'uk' ? 'active' : ''}`}>UA</button>
                <button onClick={() => { setLanguage('ru'); setLangOpen(false); }} className={`lang-opt ${language === 'ru' ? 'active' : ''}`}>RU</button>
                <button onClick={() => { setLanguage('en'); setLangOpen(false); }} className={`lang-opt ${language === 'en' ? 'active' : ''}`}>EN</button>
              </div>
            )}
          </div>
      ...
      {mobileMenuOpen && (
        <div className="mobile-menu-overlay" onClick={() => setMobileMenuOpen(false)}>
          <ul className="mobile-menu-panel" onClick={(e) => e.stopPropagation()}>
            ...
            <li><a href="#reviews" onClick={(e) => handleNavClick(e, '#reviews')}><MessageSquare size={24} />{t('nav_reviews')}</a></li>
            <li><a href="#contact" onClick={(e) => handleNavClick(e, '#contact')}><Phone size={24} />{t('nav_contact')}</a></li>
            {isAdminLoggedIn && (
              <li>
            ...
            <li className="mobile-lang-row">
              ...
              <div className="lang-switcher">
                <button className={`lang-opt ${language === 'uk' ? 'active' : ''}`} onClick={() => { setLanguage('uk'); setMobileMenuOpen(false); }}>UA</button>
